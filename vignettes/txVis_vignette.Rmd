...
Or we can align the plots so that they all appear to start at time 0, and provide some extra customization:
...
With each method it is possible to visualize treatment outcomes, and emphasise particular aspects of the treatment sequencing.  For example, understanding transitions from one treatment to another can be understood using `tx_transmat`, a visualization for transition matrices.  The method examines treatment sequences rather than dates or date ranges.  The sequences of interest can be passed using the `sequences` parameter.  By default the first two sequences are examined, although any two 


```{r}
tx_transmat(hlth_data, sequences = c(2,4))
```
